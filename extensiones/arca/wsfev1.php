...
class Wsfev1 extends WsAFIP {
	...
	public function consultarCondicionIVAReceptor() {
		$params = $this->buildBaseParams();
		try {
            $results = $this->client->FEParamGetCondicionIvaReceptor($params);
        } catch (Exception $e) {
            throw new Exception(WsAFIP::MSG_AFIP_CONNECTION . $e->getMessage(), null, $e);
        }

        $this->logClientActivity('FEParamGetCondicionIvaReceptor');
        $this->checkErrors($results, 'FEParamGetCondicionIvaReceptor');

        return $results->FEParamGetCondicionIvaReceptorResult->ResultGet->CondicionIvaReceptor;
        //return NULL;
    }
     ...
     * 
     */
    public function consultarCotizacionMoneda($id_moneda,$fchcotiz) {
        $params = $this->buildBaseParams();
        $params->MonId = $id_moneda;
		if($fchcotiz!="")
		{	
			$params->FchCotiz = $fchcotiz;
		}
        try {
    ...
    private function buildCAEAParams($voucher) {
		...
		$comprobante->CondicionIVAReceptorId = $voucher["CondicionIVAReceptorId"];
		...
		$comprobante->CanMisMonExt = $voucher["canmismaMoneda"];
    ...
    public function emitirComprobante($voucher) {
        ...
        // ✅ Capturar observaciones si existen
        $observaciones = [];
        if (isset($respuestas[0]->Observaciones)) {
            foreach ($respuestas[0]->Observaciones->Obs as $obs) {
                // Ignorar código 10245 si querés (opcional)
                if ($obs->Code == 10245) continue;

                $observaciones[] = [
                    "code" => $obs->Code,
                    "msg" => utf8_encode($obs->Msg)
                ];
            }
        }
